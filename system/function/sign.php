...
function _wap_sign($uid, $tieba){
	$cookie = get_cookie($uid);
	preg_match('/BDUSS=([^ ;]+);/i', $cookie, $matches);
	$BDUSS = trim($matches[1]);
	if(!$BDUSS) return array(-1, '找不到 BDUSS Cookie', 0);
	$ch = curl_init('http://tieba.baidu.com/mo/q/sign?tbs='.get_tbs($uid).'&kw='.$tieba['unicode_name'].'&is_like=1&fid='.$tieba['fid']);
	curl_setopt($ch, CURLOPT_HTTPHEADER, array('User-Agent: Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE72-1/021.021; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.1.16352','Referer: http://tieba.baidu.com/f?kw='.$tieba['unicode_name'] , 'Host: tieba.baidu.com','X-Forwarded-For: 115.28.1.'.mt_rand(1,255), 'Origin: http://tieba.baidu.com', 'Connection: Keep-Alive'));
	curl_setopt($ch, CURLOPT_COOKIE, $cookie);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	$sign = curl_exec($ch);
	curl_close($ch);
	preg_match('!<a name=\"top\"></a><span class=\"light\">(.*?)<br/></span>!is', $sign, $res);
	if(preg_match('!经验值上升<span class=\"light\">(.*?)</span>!is', $res[1], $exp))
	{
		$exp = $exp[1];
		return array(2, "签到成功，经验值上升 {$exp[1]}", $exp[1]);
	}
	elseif(preg_match('/亲，你之前已经签过了/', $res[1]))
		return array(2, "亲，你之前已经签过了", 0);
	else
		return array(-1, "签到失败！", 0);
}
